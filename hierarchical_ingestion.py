...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("hierarchical_parsing.log", encoding="utf-8")
    ]
)
...
class IngestionEngine:
    """
    Offline Ingestion Engine using Hierarchical Chunking.
    Best for long-form documents like reports and books.
    """
    def __init__(self, use_ocr: bool = True, use_formula: bool = False):
        logger.info("Initializing Hierarchical IngestionEngine [OFFLINE MODE]")
    ...
    def process(self, pdf_path: str, output_root: str, skip_start: int = 0, skip_end: int = 0):
        ...
        job_dir = Path(output_root) / f"{pdf_path.stem}_hierarchical_{timestamp}"
...
if __name__ == "__main__":
    print("\nDOCLING OFFLINE: HIERARCHICAL PARSER")
    engine = None
    try:
